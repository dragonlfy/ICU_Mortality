...
class HCV_NUME_WAVE_Attentive_InceptionTime1(nn.Module):
    def __init__(self, ni_w=4, nf_1=None, ni_n=4, nf_2=None, ni_hcv=37,
        ...
        self._extra_repr = (
            f"ni_w:{ni_w}, nf_1: {nf_1}, ni_n: {ni_n}, "
        ...
        k_list_1 = [5, 9, 17, 33]
        stride_1 = 4
        self.net_1 = InceptionNetwork(num_layers_1, ni_w, nf_1, stride_1, dropout_p)
        self.net_11 = InceptionNetwork1(
            num_layers_1, ni_w, nf_1, k_list_1, stride_1, dropout_p
        )
        self.linear_reducer_1 = nn.Sequential(
            nn.Conv1d(self.net_11.outc, ni_w, 1, groups=ni_w, bias=False), nn.ReLU()
        )
        self.chan_att_1 = nn.Sequential(
            nn.AdaptiveAvgPool1d(1),
            nn.Conv1d(self.net_11.outc, ni_w, 1, groups=ni_w, bias=False),
            nn.Sigmoid(),
        )
        self.time_att_1 = nn.Sequential(
            nn.Conv1d(self.net_11.outc, 1, 1, bias=False), nn.Sigmoid()
        )

        num_layers_2 = 2
        k_list_2 = [3, 5, 9]
        stride_2 = 4
        self.net_2 = InceptionNetwork(num_layers_2, ni_2, nf_2, stride_2, dropout_p)
        self.net_22 = InceptionNetwork1(
            num_layers_2, ni_2, nf_2, k_list_2, stride_2, dropout_p
        )
        self.linear_reducer_2 = nn.Sequential(
            nn.Conv1d(self.net_22.outc, ni_2, 1, groups=ni_2, bias=False), nn.ReLU()
        )
        self.chan_att_2 = nn.Sequential(
            nn.AdaptiveAvgPool1d(1),
            nn.Conv1d(self.net_22.outc, ni_2, 1, groups=ni_2, bias=False),
            nn.Sigmoid(),
        )
        self.time_att_2 = nn.Conv1d(self.net_22.outc, 1, 1, bias=False)

        self.multi_conv1d = nn.Sequential(
            nn.Conv1d(ni_hcv, nf_3, num_hours, groups=ni_hcv),
            nn.ReLU(),
            nn.Conv1d(nf_3, ni_hcv, 1, groups=ni_hcv),
            nn.BatchNorm1d(ni_hcv),
            nn.ReLU(),
        )
        ...
        self.classfier = nn.Sequential(
            nn.Linear(ni_hcv, ni_hcv),
            nn.BatchNorm1d(ni_hcv),
            nn.ReLU(),
            nn.Linear(ni_hcv, num_classes),
        )
    ...
    def forward(self, batch: MIMIC3Data):
        ...
        # wavef = self.net_11(wavei)  # shape: B*NH*L2, self.net_1.outc, 1
        # waves = wavef.reshape((B * NH, L2, self.net_11.outc))
        ...
        # output2: Tensor = self.net_22(input2)  # B*NH,net_2.outc,1
        # outs = output2.reshape((B, NH, self.net_22.outc))
        ...
        out3 = self.multi_conv1d(hcv)  # B,ni_3,1
        chan_att_3 = self.chan_att_3(hcv)
        ...
        # out3_1 = th.mean(out3, dim=1)
        out3_1 = th.mean(out3.view(-1, 37, 2), dim=2)
    ...
    def forward_cam(self, batch: MIMIC3Data) \
        ...
        out3, _ = self.bilstm(output3_att)
        out3_1 = th.mean(out3.view(-1, 51, 2), dim=2)
        out = self.classfier(out3_1)
        # out = self.classfier(output3_att)
        return out, ret_dict
...
class InceptionNetwork(nn.Module):
    def __init__(self, num_layers, ni, nf,
...
class BasicBlock(nn.Module):
    def __init__(self, ni, nf, ng,  stride, dropout_p):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv1d(ni, nf, 3, stride, padding=2, groups=ng),
            nn.BatchNorm1d(nf),
            # nn.ReLU(inplace=True)
        )
        self.conv2 = nn.Sequential(
            nn.Conv1d(nf, nf * 4, 3, stride, padding=2, groups=ng),
            nn.BatchNorm1d(nf * 4),
            # nn.ReLU(inplace=True)
        )
        self.extra = nn.Sequential(
            nn.Conv1d(ni, nf * 4, 3, stride, padding=2, groups=ng),
            nn.BatchNorm1d(nf * 4)
        )
        ...
        # self.m = nn.AdaptiveAvgPool2d(nf)

    def forward(self, input: Tensor):
        B = input.shape[0]
        x: Tensor = self.conv1(input)
        # x.reshape(B, self.outc, -1)
        print('\tconv1', x.shape)
        x = self.conv2(x)
        print(input.shape)
        # x = self.conv2(x).reshape(B, self.ng, self.nf // self.ng*4, -1)
        # print('\tconv2', x.shape)
        # x.reshape(B, self.outc, -1)
        print(x.shape[1], x.shape[2])
        self.m = nn.AdaptiveAvgPool2d((x.shape[1], x.shape[2]))
        input = self.m(input)                                                                                                 
        # x = x.reshape([7680, -1, 7500])
        # print('\tconv2', x.shape)
        # input = F.avg_pool1d(input, kernel_size=4, stride=4)
        print(input.shape, x.shape)
        out: Tensor = self.relu1(x + input)
        out = out.reshape(B, self.outc, -1)
        h1 = self.bn(out)
        h2 = self.relu(h1)
...
class InceptionNetwork1(nn.Module):
    def __init__(self, num_layers, ni, nf, k_list: list, stride=3, dropout_p=0.0):
        super(InceptionNetwork1, self).__init__()
        self.num_layers = num_layers
        self.outc = len(k_list) * nf
        module_list = [
            InceptionModule1(
                ni if idx == 0 else self.outc, nf, ni, k_list, stride, dropout_p
            )
            for idx in range(num_layers)
        ]
        self.module_list = nn.ModuleList(module_list)
...
class InceptionModule1(nn.Module):
    def __init__(self, ni, nf, ng, k_list: list, stride, dropout_p):
        ...
        self.convs = nn.ModuleList(
            [nn.Conv1d(nf, nf, k, stride, padding=k // 2, groups=ng) for k in k_list]
        )
    ...
    def forward(self, x: Tensor):
        ...
        f = self.bottleneck(x)
        f_list = [
            conv(f).reshape(B, self.ng, self.nf // self.ng, -1) for conv in self.convs
        ]
...
if __name__ == '__main__':
    ...
    batch.hcv = th.FloatTensor(size=(3, 37, 4))
    batch.demography = th.FloatTensor(size=(3, 2))
    model = HCV_NUME_WAVE_Attentive_InceptionTime1()
    x = model(batch)
    print(x.shape)
    print()
