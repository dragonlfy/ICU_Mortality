...
def calculate_single_scores (value, selector):
    ...
    # APACHEII Ranges
    #"abp":MAP 血压； temperature：体温；heart_bpm：心率；respiratory：呼吸率；oxgenation：氧含量；
    #ph，sodium：血钠含量；"potassium"：血钾；"hematocrit"，HCT：血细胞比容；"wbc"：白细胞
    #HCO3：血清碳酸氢盐
    #GCS：glasgow昏迷指数
...
#函数总共接受 6 个值来处理慢性健康点和手术状态
#value="RO NSCD">elective postoperative, no SCD (Severe Chronic Diseases, see list)选择性术后，无 SCD（严重慢性疾病，见列表）
#value="RO SCD">elective postoperative, SCD present 选择性术后，存在 SCD
#value="NO NSCD">no surgery, no SCD 无手术，无scd
#value="NO SCD">no surgery, SCD present 无手术，油scd
#value="EO NSCD">emergency postoperative, no SCD 术后紧急，无 SCD
#value="EO SCD">emergency postoperative, SCD present 术后紧急，SCD ——5分
...
print(calculate_apache2_final(60,37,90,20,300,7.7,190,7,50,22,44,"NO NSCD",1,7,15)) # should be 28