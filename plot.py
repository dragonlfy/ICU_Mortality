...
# from models.no_wave import HCV_NUME_WAVE_Attentive_InceptionTime1
from models.only_hcv import HCV_NUME_WAVE_Attentive_InceptionTime1
# from models.resnet import HCV_NUME_WAVE_Attentive_InceptionTime1
import numpy as np
...
def train(loader: DataLoader, exp: str, model: nn.Module, criterion,
    ...
    for batch in tqdm_bar:
        ...
        # 创建示例张量
        tensor1 = batch.hcv[0, :, :].transpose(0, 1)
        tensor2 = batch.nume[0, :, :]
        tensor3 = batch.wave[0, :, :]

       # 创建示例张量
        tensor1 = batch.hcv[0, :, :].transpose(0, 1)
        tensor2 = batch.nume[0:, :, :]
        tensor3 = batch.wave[0, :, :18000]
        fig, axes = plt.subplots(4, 3, figsize=(12, 16))
        fig.tight_layout()
        # 绘制张量1的四个小图
        for i in range(4):
            axes[i, 0].plot(tensor1[i].cpu().numpy())
            axes[i, 0].set_title(f'Tensor 1 - Row {i+1}')
            axes[i, 0].set_xlabel('Column index')
            axes[i, 0].set_ylabel('Value')

        # 绘制张量2的四个小图
        for i in range(4):
            axes[i, 1].plot(tensor2[i].cpu().numpy())
            axes[i, 1].set_title(f'Tensor 2 - Row {i+1}')
            axes[i, 1].set_xlabel('Column index')
            axes[i, 1].set_ylabel('Value')

        # 绘制张量3的四个小图
        for i in range(4):
            axes[i, 2].plot(tensor3[i].cpu().numpy())
            axes[i, 2].set_title(f'Tensor 3 - Row {i+1}')
            axes[i, 2].set_xlabel('Column index')
            axes[i, 2].set_ylabel('Value')

        plt.savefig('tensor_plots.png')  # 保存大图
        plt.show()
        logits = model(batch)
        logits = model(batch)
...
def run():
    ...
    config_name = 'combination1_option3'
    ...
    for fold_idx in range(0, n_splits):
        ...
        # list1 = task_config.wave_features
        # list1.pop(1)
        # list1.pop(2)
        # wavelist = list1
        # wavelist = ['ii', 'v', 'resp']
        # numelist = [task_config.nume_features[0]]
        # print(wavelist)
        # print(numelist)
